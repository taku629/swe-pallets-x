...
**3.2 Issue linking.** Each fix commit maps to its PR via the GitHub API; `fixes/closes #N` references resolve to the original issue, which becomes `problem_statement` (PR body as fallback). Real issue-derived statements (median ~1.2k chars) make tasks substantially more realistic than commit-subject stubs.
